...
So we produce 100 modifications to an endogenous ligand which has known parameters $\log_{10}(IC_{50} = -7.2$ and $bottom = 0$. We expect that the modifications might cause us to lose potency, i.e. increase $\log_{10}(IC_{50}$, and efficacy, i.e. increase $bottom$, most of the time.
...
For $top$, $n_H$, and $\sigma$, we can use the same priors as the previous studies, when we fit one curve.
