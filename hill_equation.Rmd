...
During the two studies, I avoided a rather important subject, namely assessing the quality of our posterior estimate / mcmc run

Posterior predictive shows that predictions are reasonable

All is not perfectly well
...
[Stan is noisy](https://mc-stan.org/misc/warnings.html#runtime-warnings)

Includes ways to diagnose. In addition to the above, chains mixing

```{r}
rstan::traceplot(study_1_post, alpha = 0.5)
```

What about those divergent transitions?

Increasing `adapt_delta` as suggested by Stan will help avoid the divergent transitions, but why were they there in the first place?

Let's look at some diagnostic parameters for the posterior in the first study
...
n_eff is an estimate of the number of independent samples from the posterior
Rhat is a measure of convergence. Values $\hat{R} > 1.01$ indicate that convergence has not entirely been reached.

According to the diagnostics, the marginal posterior for $n_H$ has been challenging to estimate.

Not surprising, considering that we only have one good point on the s-shaped part of the curve. There is very little information to sample from.

Why does increasing `adapt_delta` work? Intuitively, it causes Stan to pick a smaller step size for the underlying hmc sampler, which reduces the chance that the sample ends up in a far off part of parameter space where gradients are extreme. For larger models, one might have to do more samples to fully explore the posterior.
...
Let's also have a look at the diagnostics
...
There is so much more to discuss about diagnosing HMC. `?rstan::check_hmc_diagnostics` is a great place to start.
