...
```{r noisy_assay_response, fig.dim=c(8, 4), out.width="90%"}
tibble::tibble(
...
```{r prior_pred_check_example_1, fig.dim=c(8, 4), out.width="90%"}
n_samples <- 50
...
```{r posterior_marginals_example_1, fig.dim=c(8, 4), out.width="90%"}
...
```{r posterior_predictive_example_1, fig.dim=c(8, 4), out.width="90%"}
...
```{r prior_2_stan, message=FALSE, warning=FALSE, error=FALSE, results="hide", fig.dim=c(8, 4), out.width="90%"}
n_prior_samples <- 50
...
```{r posterior_marginals_study_2, fig.dim=c(8, 4), out.width="90%"}
...
Here I am fitting the data from study one using non-linear least squares. To make it fair, I have put in as much information as the opimisation algorithm allows: the initial guess is at the median of the priors we set for study one and the parameters are roughly constrained to the 95% prior interval.
