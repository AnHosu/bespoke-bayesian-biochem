...
I will be developing the bespoke model one minimally viable step at a time. In this study, I am building a Bayesian model for biochemical assays where the underlying data generating process is the Hill equation for tissue responses [@Neubig597]. I will then test the model in two simulated example studies.
...
We have observed that posterior predictive distribution yields reasonable predictions, which gives us a lot of confidence in the distributions. In these studies, we could also compare the predictions to the true underlying parameters of the simulations, so we know that the model is at least somewhat right.

All is not perfectly well, however. In study one, Stan complains that during the HMC run there were divergent transitions:

```{r}
rstan::check_divergences(study_1_post)
```

[Stan is noisy](https://mc-stan.org/misc/warnings.html#runtime-warnings) and will complain when something seems wrong in the HMC sampler.  
...
According to the diagnostics, the marginal posterior for $n_H$ has been challenging to estimate. This is not altogether surprising, considering that we only have one good point on the s-shaped part of the curve.

Imagine the situation where we are considering a value of $\log_{10}(IC_{50})$ that is very close to the data point that is on the s-shaped part of the curve. The most likely curve that satisfies this extra constraint is close to a step function, i.e. has a very large $n_H$, but this is very unlikely, according to our priors. Hence, traversing the corresponding distribution for $n_H$ is going to be difficult.
...
```{r}
rstan::check_divergences(study_1_post_improved)
```

Indeed it did!. Let's also have a look at the diagnostics.
...
Here I am fitting the data from study one using non-linear least squares. To make it fair, I have put in as much information as the opimisation algorithm allows: the initial guess is at the median of the priors we set for study one and the parameters are constrained to the 95% prior interval.
...
In the preceding two studies, we spent a lot of time fuzzing over just a few data points. In real applications, however, data is often more abundant and more diverse. I am thinking of screening experiments, in particular. In screening experiments, one might test a large number of potential ligands at once, meaning that there is more data but also more behaviour to be captured.
