...
example_1_prior_pred_samples <- example_1_priors(n_samples) %>%
...
Reduce(`+`, example_1_prior_pred_samples$tissue_response, init = p)
```
...
example_1_concentrations <- seq(-9, -3, length.out = n_observations)

example_1_observations <- rlang::exec(
  assay_response,
  example_1_concentrations,
  !!!example_1_params
)
...
parameter_tibble <- example_1_params %>%
  tibble::as_tibble() %>%
  tidyr::pivot_longer(
    dplyr::everything(),
    names_to = "parameter",
...
samples %>%
  ...
  labs(y = "Posterior sample count", x = "", colour = "", fill = "")
```
...
p <- ggplot() + theme_minimal() + labs(x = "Ligand concentration [M]" , y = "Prior assay response")
